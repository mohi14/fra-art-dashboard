...
const MailConfiguration = () => {
  ...
  const searchFilter = (mail) => {
    if (searchText) {
      return mail.account.toLowerCase().includes(searchText.toLowerCase());
    } else {
      return true;
    }
  };
            ...
            <div className="overflow-x-auto">
              <table className="table-auto w-full divide-y divide-slate-200">
                ...
                <tbody className="text-sm">
                  {mailConfiguration &&
                    mailConfiguration.length > 0 &&
                    mailConfiguration.filter(searchFilter).map((mail) => (
                      <tr>
                        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                          <div>{mail?.account}</div>
                        </td>
                        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                          <div>{mail?.password}</div>
                        </td>
                        ...
                        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                          <div>{mail?.senderServer}</div>
                        </td>

                        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                          <div>{mail?.receivingServer}</div>
                        </td>
                        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                          <div>
                            {mail?.whetherTheDefault === false ? "No" : "Yes"}
                          </div>
                        </td>
                        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                          <div>{mail?.sender}</div>
                        ...
                        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">
                          <div className="space-x-1">
                            <Link
                              to={`/system/edit-mail-configuration/${mail?._id}`}
                            >
